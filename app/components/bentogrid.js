				...
				<div className='grid grid-cols-2 gap-4 sm:grid-cols-3 md:gap-6 xl:gap-8'>
					{galleryItems.map((item) => (
						<Link
							key={item.id}
							href={item.href}
							className={`group relative flex h-48 items-end overflow-hidden rounded-3xl bg-muted shadow-lg border border-border/30 ${
								item.span || ''
							...
							{/* Gradient overlay */}
							<div className='pointer-events-none absolute inset-0 bg-linear-to-t from-background/80 via-background/30 to-transparent opacity-80' />
